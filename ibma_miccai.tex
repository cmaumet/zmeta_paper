...
Section~\ref{sec_meth} describes the 8 meta-analytic estimates along with the experiment undertaken on simulated and real data to assert their validity. The results are described in section~\ref{sec_res}. Finally, we conclude in section~\ref{sec_ccl}.
...
In a fixed-effects (FFX) model (i.e. assuming no or negligible between-study variance), we have $W = \mathrm{diag}( \varWithin[1] \ldots \varWithin[\nStudies] )$ where $\varWithin$ denotes the contrast variance for study $i$. This approach will be referred to as \textbf{FFX GLM}.

\begin{table}[t]
\begin{center}
\begin{tabular}{cccc}
				& Meta-analysis statistic			& Distribution under H$_0$ & Study inputs \\
\hline						
...
MFX GLM 		& $ \left( \sum_{i=1}^\nStudies \frac{\effect}{\vareffect + \hat\varBetween} \right)/ \sqrt{\sum_{i=1}^\nStudies 1/ (\vareffect + \hat\varBetween) } $ & $\mathcal{T}_{\nStudies - 1}$ & $\effect, \vareffect$\\
...
\end{tabular}
\end{center}
\caption{Statistics for one-sample meta-analysis tests and distributions under the null hypothesis. (*) determined using permutations with sign flipping.}
\label{stat_table}
\end{table}	
...
As an alternative to parametric approaches, non-parametric inference~\cite{Holmes1996,Nichols2002} can be performed by comparing the RFX GLM T-statistic to the distribution obtained ``sign flipping'', i.e.\ randomly multiplying each study's data by 1 or -1. This approach will be referred to as \textbf{Contrast permutation}.
...
If their underlying assumptions are verified the meta-analytic estimators will exactly follow the null distribution specified in Table~\ref{stat_table}. Their behaviour under violation of these assumptions however needs to be carefully assessed to identify which approach is suitable for neuroimaging. Based on simulations, we verified the validity of each estimator under inhomogeneity of contrast variances $\vareffect$ and under the presence of non-negligible between-study variance.
...
\end{eqnarray}
where $\varWithin \in [1/2, 1, 2, 4]$ is the within-study variance, $\varBetween \in [0, 1/20]$ is the between-study variance (fixed-effects models are strictly only appropriate for $\varBetween=0$). For different number of studies per meta-analysis: $\nStudies \in [5, 10, 25, 50]$, we selected the number of subjects per studies $\sampleSize$ in order to have varying number of subjects per studies in a given meta-analysis across the common range of subjects involved in neuroimaging studies. In each simulated meta-analysis we simulated one study with exactly 20, 25, 10 and 50 subjects. For the remaining studies $1/4$ of the $\sampleSize$'s were drawn from $\mathcal{U}(11,20)$, $1/4$ from $\mathcal{U}(26,50)$ and the remaining from $\mathcal{U}(21,25)$, where $\mathcal{U}(a,b)$ is the discrete uniform distribution on the integers $a$ to $b$ inclusive. A total of 32 parameter sets (4 $\varWithin$ x 2 $\varBetween$ x 4 $\nStudies$) was therefore tested and a total of $71^3$ realisations were created.


\subsubsection{Real data}
We then verified the adequacy of the results provided by the 8 meta-analytic estimators to the reference approach, MFX GLM, on a dataset of 21 studies of pain. 
Comparability of contrast estimates depends on equivalent scaling of the data, models, and contrast vectors. Data scaling was consistent performed by FSL, setting median brain intensity to 10,000; model were all created by FSL's Feat tool; and contrasts were constructed to preserve units, with sum of positive elements equal to 1, sum of negative elements equal to -1. 
...
To investigate the presence of between-study variations, we computed the ratio of the between-study variance (estimated using FSL's FLAME~\cite{Smith2001}) to the total variance (sum of between- and within-study variances) as suggested as a measure of heterogeneity in~\cite{Chen2012}. Here we use the average (across study) within-study variance as an estimate of within-study variance in the denominator, leading to:
$\frac{\estvarBetween}{\estvarBetween + \sum_{i=1}^{\nStudies} \vareffect}$.
...
Fig.~\ref{fig_realdata} plots the difference between the z-score estimated by each meta-analytic approach and the reference z-score computed with MFX GLM. All FFX statistics provide overly optimistic z-estimate suggesting, again, that study heterogeneity is present in the studied dataset.
...
We have compared eight meta-analytic statistic in the context of one-sample test. Through simulations, we outlined the invalidity of standard FFX approaches in the presence of study heterogeneity. In a real dataset of 21 studies of pain, we outline the heterogeneity advocating for the use of RFX meta-analytic statistics. When only contrast estimates are available, RFX GLM was valid. This is in line with previous results on within-group one-sample t-tests studies~\cite{Mumford2009}. When only standardised estimates are available, permutation is the preferred option as the one providing the most faithful results. Further investigations are needed in order to assess the behaviour of these estimators in other configurations, including meta-analyses focusing on between-study differences.
