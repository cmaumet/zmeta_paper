...
\subsection{Robustness to small samples}

\begin{figure}[h]
	\centering
 	\includegraphics[trim={0 3cm 0 3cm},clip,width=0.8\linewidth]{../zmeta/small_samples/test1_smallsample_robustness_p.pdf}
	\caption{Deviation from theoretical P-values in one-sample tests under ideal circunstances with respect to heterogeneity for each statistical approach ($\varBetween=0$ for FFX and $\varBetween=1$ for MFX) and $\nStudies = 5, 25, 50$ with matched (``nominal'') units.}
	\label{small_sample_test1}
\end{figure}
...
\subsection{Robustness to heteroscedasticity}

\begin{figure}[h]
	\centering
 	\includegraphics[trim={0 0 0 0
 	},clip,width=0.8\linewidth]{../zmeta/small_samples/robustness.pdf}
	\caption{Deviation from theoretical P-values for a range of varying within-study variances with $\nStudies = 25$ and $n = 20$ with matched (``nominal'') units.}
	\label{heteroscedasticity}
\end{figure}

RFX and Perm E. are robust to heteroscedasticity for all settings studied.

\subsection{Robustness to heterogeneity}
