...
Neuroscience research studies typically exhibit low power and therefore have low likelihood that a given result is True~\cite{Button2013}. Under low power, both true effects are less likely to be detected and, when an effect is detected, it is less likely to be true~\cite{Button2013}. Many authors have advocated for increased sample sizes in neuroimaging (e.g. ~\cite{Poldrack2014}) which would mechanically increase the power. Aware of this issue, our community has started a couple of years ago to generated large datasets by means of consortia (e.g. ADDREF 1000FC) or dedicated datasets (e.g. ADDREF UK biobank) which are starting to show promising results. Another approach to increase sample size, is to combine the results of previous studies through meta-analyses. Functional Magnetic Resonance Imaging (fMRI), with its ample literature of more than 100 000 articles [ADDREF] is particularly well-suited for this approach. -- use mature field with robust well-established methods

% A growing literature is focusing on the lack of statistical power in neuroimaging studies (see, e.g.~\cite{Button2013}), feeding the debate on the validity and reproducibility of published neuroimaging results. Meta-analysis, by providing inference based on the results of previously conducted studies, provides an essential method to increase power and hence confidence in results. 

Most fMRI meta-analyses are `'coordinate-based'', i.e. they proceed by combining information about the locations of local maxima (and optionnally their statistic value). But, the best practice method is an Image-Based Meta-Analysis (IBMA) that combines information from each study at every location in space~\cite{Salimi-khorshidi2009}, effectively enabling the discovery of consistent sub-threshold effects that could not be uncover using coordinates only.
For a long time, the standard in the fMRI literature was to report a table with coordinates and statistic values of each local maxima. IBMAs were very limited as they required contacting back the original authors of each study of interest to retreive the summary statistic images, an approach that is both time-consuming and inefficient (ADD REF on data availability). With the arrival of tools that facilitate sharing 3D volumes obtained as a result of a statistical analysis, such as NeuroVault~\cite{Gorgolewski2015}, IBMAs becomes more and more easy to perform.

IBMAs can be performed with three different types of summary data per study:
\begin{enumerate}
...
\end{enumerate}
But all IBMA methods have specific limitations. The first option (\ES), which is considered best practice, leading to more statistically efficient estimates~\cite{Cummings2004}, requires the contrasts to be expressed with in the same units. In fMRI, units will depends on the field strength~\cite{Chen2016} as well as data, model and contrast vector scaling~\cite{Nichols2012units}. Also, while often neglected, the statistical methods for this first option rely on asymptotic, large sample results and may have poor performance for typical number of studies [ADDREF,ADDREF].
The second option (\E) assumes homoscedasticity, i.e. constant variance of the study-level contrast estimates, an assumption that is difficult to atteign in meta-analysis where the number of subjects is likely to vary across studies.
Finally the third option (\Z) corresponds to historical methods that typically assume homogeneity, i.e. no between-study variance. 

Given the growing interest in data sharing in the neuroimaging community~\cite{Poldrack2014,Nichols2017}, there is a need to identify what is the best approach for neuroimaging IBMAs and what is the minimal data needed. Here, we compared the use of IMBA using 9 meta-analytic approaches: 2 approaches use \ES's, 2 \E's only and 5 \Z's. We assessed and compared validity using simulated data under violation of each method's underlying assumptions: in small samples, under hetreroscedasticity, under heterogeneity or with contrast using mismatched units. We estimated the accuracy using a real dataset of 21 studies of pain.
