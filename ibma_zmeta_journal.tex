...
In the most general case of a random-effects meta-analysis, i.e. assuming non-zero between-study variance, $W$ is a diagonal matrix with entries $\varBetween + \varWithinCon$ where $\varBetween$ denotes the between-study variance and $\varWithinCon$ denotes the variance of the contrast estimate for study $i$. Eq.~\eqref{eq_meta_GLM} can be solved by weighted least squares giving:
\begin{eqnarray}
...
\begin{eqnarray}
	...
	\label{eq_FGLS}
\end{eqnarray}
The error variance is estimated $\hat{W}_i = \vareffect+\estvarBetween, \estvarBetween$ estimated from `FLAME~1', MoM or metafor [TODO]. Although FGLS is asymptotically efficient, its finite sample properties are unknown~\cite{Greene2012} and there is no exact null distribution. Inference from `FLAME~1' used a Student distribution with $\nStudies-\nMetaAnalyticEffects$ degrees of freedom~\cite{Woolrich2004} as depicted in Table~\ref{table:stats_test1}. This reference approach will be referred to as \textbf{Mixed-effects (MFX) GLM}.

% TODO: is there a better ref for FFX dof than FEAT user manual?
In a fixed-effects meta-analysis, i.e. assuming no or negligible between-study variance, $W$ is a diagonal matrix with entries $\varWithinCon$. We obtained a FGLS estimate by approximating $\varWithinCon$ by $\vareffect$. Inference used a Student distribution with $(\sum_{i=1}^\nStudies (\nSubjects - 1)) -\nMetaAnalyticEffects$ degrees of freedom (with `Simple OLS' in FSL~\cite{FEATUserGuide}) as depicted in Table~\ref{table:stats_test1}. This reference approach will be referred to as \textbf{Fixed-effects (FFX) GLM}.
...
If the $\vareffect$ are unavailable, the contrast estimates $\effect$ can be combined by assuming that the within-study contrast variance is negligible in comparison to the between-study variance ($\varWithinCon \ll \varBetween$) or that $\varWithinCon$ is constant ($\varWithinCon = \sigma^2 \;\; \forall i$). Then $W$ has a constant diagonal equal to $\varCombined$, where $\varCombined$ combines the within and between-study variances, i.e.\ $\varCombined \simeq \varBetween$ in the former case or $\varCombined \simeq \varBetween + \varWithinCommon$ in the latter, though note that we do not separately estimate $\varBetween$ or $\varWithinCommon$. Under these assumptions, Eq.~\eqref{eq_meta_GLM} can be solved by ordinary least squares giving:
...
When only test statistic images are available there are a several alternative approaches available. \textbf{Fisher}'s meta-analysis provide a statistic to combine the associated p-values~\cite{Fisher1932}. \textbf{Stouffer}'s approach combines directly the standardised statistic~\cite{Stouffer1949}. It has also been proposed to weights each study's $\zeffect$ by the square root of its sample size~\cite{Zaykin2011}, in an approach we'll refer to as \textbf{Weighted Z}. These three methods are all fixed effects methods and do not account for between-study variance; further, they only apply for a one-sample model (no meta-regression). The corresponding statistics and nominal null distributions are presented in Table~\ref{table:stats_test1}.
